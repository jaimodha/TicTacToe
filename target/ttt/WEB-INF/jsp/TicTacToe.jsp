...
<c:otherwise>
<td><strong><c:if test="${flag}"><a href = "TicTacToe.html?id=${row}"></c:if><font size="18">${pboard[row]}</font><c:if test="${flag}"></a></c:if></strong></td>
</c:otherwise>
</c:choose>
</c:forEach>
</tr>
...
<li><a href = "savegame.html">Save Game</a></li>
</c:if>
</c:when>
<c:otherwise>
